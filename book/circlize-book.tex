...
greatly enhances the visualization of scientific results. Thus, plots
with circular layout are normally named as \textbf{``circos plot''}.
Here the \textbf{circlize} package aims to implement \textbf{Circos} in
R. One important advantage for the implementation in R is that R is an
ideal environment which provides seamless connection between data
analysis and data visualization. \textbf{circlize} is not a front-end
wrapper to generate configuration files for \textbf{Circos}, while
completely coded in R style by using R's elegant statistical and graphic
engine. We aim to keep the flexibility and configurability of
\textbf{Circos}, but also make the package more straightforward to use
and enhance it to support more types of graphics.

In this book, chapters in Part I give a detailed overview of the general
...
\tightlist
\item
  \texttt{circos.points()}: add points in a cell.
\item
  \texttt{circos.lines()}: add lines in a cell.
\item
  \texttt{circos.segments()}: add segments in a cell.
\item
  \texttt{circos.rect()}: add rectangles in a cell.
\item
  \texttt{circos.polygon()}: add polygons in a cell.
\item
  \texttt{circos.text()}: add text in a cell.
\item
  \texttt{circos.axis()} and \texttt{circos.yaxis()}: add axis in a
  cell.
\end{itemize}
...
\tightlist
\item
  \texttt{circos.track()}: create plotting regions for cells in one
  single track.
\item
  \texttt{circos.update()}: update an existed cell.
\item
  \texttt{circos.par()}: graphic parameters.
\item
  \texttt{circos.info()}: print general parameters of current circular
  plot.
\item
  \texttt{circos.clear()}: reset graphic parameters and internal
  variables.
\end{itemize}
...
(in the polar coordinate) and go along the circle clock-wisely. You may
not see anything after running following code because no track has been
added yet.
...
have a default height of 0.1. The circle always has a radius of 1, so a
height of 0.1 means 10\% of the circle radius.

Note that the allocation of sectors only needs values on x direction (or
circular direction), the values on y direction (radical direction) will
be used in the step of creating tracks.
...
\NormalTok{            CELL_META}\OperatorTok{$}\NormalTok{sector.index)}
        \KeywordTok{circos.axis}\NormalTok{()}
\NormalTok{\})}
...
\texttt{panel.fun} (see code above). \texttt{circos.track()} creates
...
in the ``current cell'' (Functionality of \texttt{panel.fun} is further
discussed in Section @ref (panel-fun)). Without specifying any
arguments, \texttt{circos.axis()} draws x-axes on the top of each cell
(or the outside of each cell).
...
\texttt{circos.initializeWithIdeogram()} and \texttt{chordDiagram()}
do).
...
We add histograms to the second track. Here \texttt{circos.trackHist()}
is a high- level function which means it creates a new track (as you can
imagin \texttt{hist()} is also a high-level function). \texttt{bin.size}
is explicitly set so that the bin size for histograms in all cells are
the same and can be compared to each other. (See Figure
\ref{fig:circlize-glance} B)
...
\texttt{panel.fun} are exactly values in the ``current'' cell. (See
Figure \ref{fig:circlize-glance} C)
...
it. (See Figure \ref{fig:circlize-glance} D)
...
\texttt{NULL}). (See Figure \ref{fig:circlize-glance} E)
...
{\centering \includegraphics{01-introduction_files/figure-latex/circlize-glance-1} 

}

\caption{A step-by-step example}\label{fig:circlize-glance}
\end{figure}
...
circular plot is always (or mostly except you want to draw something out
of the circle) drawn inside the circle which has radius of 1 (which
means it is always a unit circle), and from outside to inside}.
...
\item
  ...
  index is what the two function expect and it can be ommited when
...
\item
  ...
  that you can directly use low-level functions without specify sector
  index and track index.
...
the blue represent one sector. The intersection of a sector and a track
...
category. The width of sectors are proportional to the data range in
sectors on x direction. The data range can be specified as a numeric
vector \texttt{x} which has same length as \texttt{factors}, then
\texttt{x} is split by \texttt{factors} and data ranges are calculated
for each sector internally.
...
data range on y direction for cells. Similar as
\texttt{circos.initialize()}, \texttt{circos.track()} also receives
...
In Section \ref{panel-fun}, we will introduce how \texttt{x} and
\texttt{y} are sent to each cell and how the graphics are added.
...
initialization step. If users only create cells for a part of sectors in
the track (not all sectors), in fact, cells in remaining unspecified
...
\CommentTok{# You will only see cells that are covered in `factors`}
\KeywordTok{circos.track}\NormalTok{(factors, }\DataTypeTok{y =}\NormalTok{ y)}
\CommentTok{# You will see all cells are covered}
...
\item
  ...
  which means it is always reverse clockwise. E.g. if it is set to 90,
  sectors start from the top center of the circle. See Figure
...
\item
  ...
  has same number as sectors. ** Note the first gap is after the first
  sector.** See Figure \ref{fig:circlize-direction} and figure
...
\item
  \texttt{track.margin}: Like \texttt{margin} in Cascading Style Sheets
  (CSS), it is the blank area out of the plotting region, also outside
  of the borders. Since left and right margin are controlled by
  \texttt{gap.after}, only bottom and top margin need to be set. The
  value for \texttt{track.margin} is the percentage to the radius of the
  unit circle. The value can also be set by \texttt{convert\_height()}
  or the short version \texttt{uh()} function with absolute units. See
  figure \ref{fig:circlize-region}.
\item
  \texttt{cell.padding}: Padding of the cell. Like \texttt{padding} in
  Cascading Style Sheets (CSS), it is the blank area around the plotting
  regions, but within the borders. The parameter has four values, which
  control the bottom, left, top and right padding respectively. The
  first and the third padding values are the percentages to the radius
  of the unit circle, and the second and fourth values are the degrees.
  The first and the third value can be set by \texttt{uh()} with
...
\item
  ...
  \ref{fig:circlize-glance} A). Set this value to \texttt{FALSE} to turn
  off the warnings.
\item
...
low- level graphics on it. The minimal set of arguments for this
function is to set either \texttt{y} or \texttt{ylim} which assigns
range of y values for this track. \texttt{circos.track()} creates tracks
for all sectors although in some case only parts of them are visible.
...
equal to the number of sectors. Thus, you can create plot regions with
different styles of borders and background colors.
...
conversion is applied in the ``current'' cell, but it can still be
specified by \texttt{sector.index} and \texttt{track.index} arguments.
Since the width of the cell is not the same from the top to the bottom
in the cell, for \texttt{convert\_x()} or \texttt{ux()} function, the
position on y direction where the convert is applied needs to be
specified. By default it is at the middle point on y-axis.
...
\texttt{circos.info()}. The function can be called any time.
...
It can also add labels to cells by \texttt{circos.info(plot\ =\ TRUE)}.
...
read. To make text more easy to read and not to hurt users' neck too
...
bottom edge become two arcs. This function can be vectorized.
...
a cell. Please note the first data point shoud overlap to the last data
point.
...
As you can notice in the above figure, when the first and last axis
labels exceed data ranges on x-axis in the corresponding cell, their
positions are automatically adjusted to be shifted towards in the cell.
...
the two sectors are all single points, the link represents a line. If
...
{\centering \includegraphics{03-graphics_files/figure-latex/unnamed-chunk-17-1} 

}

\caption{Positions of link ends.}\label{fig:unnamed-chunk-17}
\end{figure}
...
text for the highlighted regions. By default, the text is drawn at that
...
a string in a form of ``2 inches'' or ``-1.2cm''.
...
\section{Work with the base graphic
...
by using functionality from other R packages, it is just a few more work
to really implement it. Here I will demonstrate how to customize legends
and arrange to the circular plot.
...
Actually you can see that one big problem is if there are many legends
...
plotting regions (or, viewport in \textbf{grid} term), and
...
\chapter{Implement high-level plots}\label{high-level-plots}
...
\texttt{circos.dendrogram()} (Figure @ref(fig:phylogenetic\_tree) left).
...
Note, we must change the sector name in the zoomed data frame. We just
simply add ``zoom\_'' prefix to the original name to show it is a
``zoomed'' sector. After that, it is attached to the original data
frame.
...
circle and the zoomed sectors in the right. Here is where the magic
thing happens. We simply normalize the width of normal sectors and
normalize the width of zoomed sectors separately. Now the sum of the
sector width for the original sectors is 1 and the sum of sector width
for the zoomed sectors is 1, which means these two types of sectors now
have their own half circle.
...
Now just make the circular plot in the normal way. All the graphics in
sector a and b will be automatically zoomed to sector ``zoomed\_a'' and
``zoomed\_b''.
...
show where the zooming happens (Figure \ref{fig:circlize-zoom}). Notice
...
\NormalTok{    factors =}\StringTok{ }\DecValTok{1}\OperatorTok{:}\DecValTok{8}
    ...
    \KeywordTok{circos.par}\NormalTok{(}\DataTypeTok{cell.padding =} \KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{0}\NormalTok{))}
    \KeywordTok{circos.initialize}\NormalTok{(factors, }\DataTypeTok{xlim =} \KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{))}
    \KeywordTok{circos.trackPlotRegion}\NormalTok{(}\DataTypeTok{ylim =} \KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{), }\DataTypeTok{track.height =} \FloatTok{0.05}\NormalTok{,}
...
with the corresponding regions. Each genomic region are composed by
three elements: genomic category (in most case, it is the chromosome),
start position on the genomic category and the end position. Such data
...
@ref(fig:genomic-initialize- ideogram) A). By default, the function
...
lengths. The sumation of such large integers would throw error of
integer overflow.
...
chromosomes (Figure \ref{fig:genomic-initialize-ideogram} B).
...
When there is no cytoband data for the specified species, and chromInfo
data is used instead, they may be many many extra short contigs.
\texttt{chromosome.index} can also be useful to remove unnecessary
contigs.
...
tracks to add. (figure Figure \ref{fig:genomic-initialize-ideogram} C,
D).
...
\ref{fig:genomic-initialize-ideogram} E, F).
...
{\centering \includegraphics{08-initialize-genomic-data_files/figure-latex/genomic-initialize-ideogram-1} 

}

\caption{Initialize genomic plot.}\label{fig:genomic-initialize-ideogram}
\end{figure}
...
and \texttt{circos.genomicRect()} will be discussed in Sector
...
becomes ``0KB'' while not the correct values. It is just an adjustment
...
\texttt{read.cytoband()} downloads and reads cytoband data from UCSC. In
following, x ranges for normal chromosomes and zoomed chromosomes are
normalized separetely.
...
\KeywordTok{circos.genomicTrackPlotRegion}\NormalTok{(}\KeywordTok{extend_chromosomes}\NormalTok{(bed, }\KeywordTok{c}\NormalTok{(}\StringTok{"chr1"}\NormalTok{, }\StringTok{"chr2"}\NormalTok{)),}
    \DataTypeTok{panel.fun =} \ControlFlowTok{function}\NormalTok{(region, value, ...) \{}
...
data}\label{create-plotting-regions-for-genomic-data}
...
\begin{verbatim}
##    chr  start    end      value1    value2
## 1 chr1 219934 283082 -0.06978513 0.3899233
## 2 chr1 359777 406208 -0.08276002 0.3339521
\end{verbatim}
...
## 2 359777 406208
##        value1    value2
## 1 -0.06978513 0.3899233
## 2 -0.08276002 0.3339521
\end{verbatim}
...
(named index or numeric index, can be a vector with more than one
columns) or directly set \texttt{ylim}. If none of them are set, the
...
\texttt{circos.points()}. \texttt{circos.genomicPoints()} expects a data
frame which contains genomic regions and a data frame containing
corresponding values. Points are always drawn at the middle of each
region. The data column of the y values for plotting should be specified
by \texttt{numeric.column}. If \texttt{numeric.column} has length larger
than one, all the specified columns will be used to add points.
...
\KeywordTok{circos.genomicTrack}\NormalTok{(data, }\DataTypeTok{numeric.column =} \DecValTok{4}\NormalTok{, }
    \DataTypeTok{panel.fun =} \ControlFlowTok{function}\NormalTok{(region, value, ...) \{}
        \CommentTok{# numeric.column is changed to 1}
        \KeywordTok{circos.genomicPoints}\NormalTok{(region, value, ...)}
\NormalTok{\})}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{    y =}\StringTok{ }\NormalTok{value[[numeric.column]]}
    \KeywordTok{circos.points}\NormalTok{(x, y, ...)}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{circos.genomicTrackPlotRegion}\NormalTok{(bed, }\DataTypeTok{ylim =} \KeywordTok{c}\NormalTok{(}\OperatorTok{-}\DecValTok{1}\NormalTok{, }\DecValTok{1}\NormalTok{),}
    \DataTypeTok{panel.fun =} \ControlFlowTok{function}\NormalTok{(region, value, ...) \{}
        \KeywordTok{circos.genomicPoints}\NormalTok{(region, value, ...)}
...
the current data frame. The index for the current data frame can be get
by \texttt{getI(...)}. Note \texttt{getI(...)} can only be used inside
\texttt{panel.fun} and \texttt{...} argument is mandatory.
...
setting \texttt{stack\ =\ TRUE}. Under \texttt{stack} mode, Graphics are
vertically or radically arranged line by line for each dataset.
...
\begin{Highlighting}[]
\NormalTok{data =}\StringTok{ }\KeywordTok{generateRandomBed}\NormalTok{(}\DataTypeTok{nc =} \DecValTok{2}\NormalTok{)}
\KeywordTok{circos.genomicTrackPlotRegion}\NormalTok{(data, }\DataTypeTok{stack =} \OtherTok{TRUE}\NormalTok{,}
    \DataTypeTok{panel.fun =} \ControlFlowTok{function}\NormalTok{(region, value, ...) \{}
\NormalTok{        i =}\StringTok{ }\KeywordTok{getI}\NormalTok{(...)}
        \KeywordTok{circos.genomicPoints}\NormalTok{(region, value, }\DataTypeTok{col =}\NormalTok{ i, ...)}
\NormalTok{\})}
...
\begin{Highlighting}[]
\NormalTok{bed_list =}\StringTok{ }\KeywordTok{list}\NormalTok{(}\KeywordTok{generateRandomBed}\NormalTok{(), }\KeywordTok{generateRandomBed}\NormalTok{())}
\KeywordTok{circos.genomicTrackPlotRegion}\NormalTok{(bed_list, }\DataTypeTok{stack =} \OtherTok{TRUE}\NormalTok{,}
    \DataTypeTok{panel.fun =} \ControlFlowTok{function}\NormalTok{(region, value, ...) \{}
\NormalTok{        i =}\StringTok{ }\KeywordTok{getI}\NormalTok{(...)}
        \KeywordTok{circos.genomicPoints}\NormalTok{(region, value, ...)}
\NormalTok{\})}
...
columns, graphics on all horizontal lines share the same genomic
positions while if using a list of data frames, the genomic positions
can be different.

\section{Applications}\label{applications}

In this section, we will show several real examples of making genomic
plots under different modes. Again, if you are not happy with these
...
\KeywordTok{circos.genomicTrackPlotRegion}\NormalTok{(bed_list, }
    \DataTypeTok{panel.fun =} \ControlFlowTok{function}\NormalTok{(region, value, ...) \{}
\NormalTok{        i =}\StringTok{ }\KeywordTok{getI}\NormalTok{(...)}
...
mode. Graphics corresponding to each data frame is added to a horizontal
line.
...
\subsection{Lines}\label{lines-1}
...
\subsection{Rectangles}\label{rectangles-1}
...
\texttt{circos.genomicIdeogram()} creates a thin track of ideograms and
...
heatmaps. Heatmaps complete fill the track and there are connection
lines connect heatmaps and positions in the genome.
`circos.genomicHeamtap() draws connection lines and heatmaps as two
tracks and combines them as a integrated track.

Generally, all numeric columns (excluding the first three columns) in
the input data frame are used to make the heatmap. Colors can be
specfied as a color matrix or a color mapping function generated by
\texttt{colorRamp2()}.
...
\texttt{circos.genomicLabels()} adds text labels for regions specified.
...
\texttt{labels\_track\ =\ max(strwidth(labels))}. \texttt{padding}
...
to be zoomed is too many, the method will not work, and in this chapter,
we introduce another zooming method which put zoomed regions in a
different circle.
...
an independent track, thus, the name for each zoomed interval uses
...
original track and one for the zoomed intervals, and one connection
track which identifies which intervals that are zoomed belong to which
sector. The \texttt{circos.nested()} function in \textbf{circlize} puts
the two circular plots together, arranges and rotates them.
...
\texttt{corresponance} together to \texttt{circos.nested()} (Figure
\ref{fig:nested} left).
...
which one is zoomed or not, they are just two circular plots (Figure
\ref{fig:nested} middle).
...
\item
  ...
  adjusted to make the difference between two plots to a minimal.
  However, users can also manually adjusted the start degree for the
  second plot by setting \texttt{circos.par("start.degree"\ =\ ...)} and
  \texttt{adjust\_start\_degree} must be set to \texttt{TRUE} in
  \texttt{circos.nested()}.
\item
  Since the function needs to know some information for the two circulat
...
circular plot. It is no problem to make it more complex (Figure
\ref{fig:nested} right).
...
\NormalTok{            \}}
            \KeywordTok{circos.points}\NormalTok{(x, y, }\DataTypeTok{pch =} \DecValTok{16}\NormalTok{, }\DataTypeTok{cex =} \FloatTok{0.5}\NormalTok{)}
            \KeywordTok{circos.text}\NormalTok{(CELL_META}\OperatorTok{$}\NormalTok{xcenter, CELL_META}\OperatorTok{$}\NormalTok{ylim[}\DecValTok{2}\NormalTok{] }\OperatorTok{+}\StringTok{ }\KeywordTok{uy}\NormalTok{(}\DecValTok{2}\NormalTok{, }\StringTok{"mm"}\NormalTok{), CELL_META}\OperatorTok{$}\NormalTok{sector.index, }
                \DataTypeTok{niceFacing =} \OtherTok{TRUE}\NormalTok{, }\DataTypeTok{adj =} \KeywordTok{c}\NormalTok{(}\FloatTok{0.5}\NormalTok{, }\DecValTok{0}\NormalTok{))}
\NormalTok{    \})}
\NormalTok{\}}
...
{\centering \includegraphics{12-nested-zooming_files/figure-latex/nested-1} 

}

\caption{Nested zooming between two circular plots.}\label{fig:nested}
\end{figure}
...
examine only a minor fraction of methylome of interest. Circular plot
can be used to
\href{http://jokergoo.github.io/circlize/example/railfallplot.html}{visualize
genome-wide distribution of differentially methylation regions (DMRs)}.
In this section, we demonstrate how to visualize DMRs which are detected
from T-WGBS data in a circular plot by \textbf{circlize} package.
...
tagment regions. Correspondance between tagment regions and DMRs can be
checked by row names of \texttt{tagments} and \texttt{tagment} column in
\texttt{DMR1}.
...
\NormalTok{f2 =}\StringTok{ }\ControlFlowTok{function}\NormalTok{() \{}
    ...
    \KeywordTok{circos.genomicTrack}\NormalTok{(DMR1, }\DataTypeTok{ylim =} \KeywordTok{c}\NormalTok{(}\OperatorTok{-}\FloatTok{0.6}\NormalTok{, }\FloatTok{0.6}\NormalTok{), }\DataTypeTok{panel.fun =} \ControlFlowTok{function}\NormalTok{(region, value, ...) \{}
...
\NormalTok{        \}}
        ...
        \KeywordTok{circos.genomicPoints}\NormalTok{(region, value, }\DataTypeTok{col =} \KeywordTok{ifelse}\NormalTok{(value[[}\DecValTok{1}\NormalTok{]] }\OperatorTok{>}\StringTok{ }\DecValTok{0}\NormalTok{, }\StringTok{"#E41A1C"}\NormalTok{, }\StringTok{"#377EB8"}\NormalTok{), }
...
straightforward or customized way.
...
be another graphic mapping for categorize relations; 4. width of sectors
represents total strength for a link which connects to other objects or
is connected from other objects. You can find an interesting example of
...
\texttt{mat} (Figure \ref{fig:chord-diagram-basic} left).
...
correspond to rows but with 50\% transparency.
...
\ref{fig:chord-diagram-par}, left). It is useful when rows and columns
belong to different categories. Please note since you change default
graphical settings, you need to use \texttt{circos.clear()} in the end
of the plot to reset it.
...
starts from right center of the circle and sectors are arranged clock
wise. The start degree for the first sector can be set by
\texttt{circos.par(start.degree\ =\ ...)} and the direction can be set
by \texttt{circos.par(clock.wise\ =\ ...)} (Figure
\ref{fig:chord-diagram-par}, right).
...
{\centering \includegraphics{13-chord-diagram-simple_files/figure-latex/chord-diagram-par-1} 

}

\caption{Control `chordDiagram()` by `circos.par()`.}\label{fig:chord-diagram-par}
\end{figure}
...
(Figure \ref{fig:chord-diagram-color} A). The value should between 0 to
1 in which 0 means no transparency and 1 means full transparency.
...
will be ignored if it is set (Figure \ref{fig:chord-diagram-color} B).
...
generated by \texttt{colorRamp2()} (Figure \ref{fig:chord-diagram-color}
C).
...
the third column has the correct values.
...
(Figure \ref{fig:chord-diagram-color} D, E). Here note values of colors
can be set as numbers, color names or hex code, same as in the base R
graphics.
...
{\centering \includegraphics{13-chord-diagram-simple_files/figure-latex/chord-diagram-color-1} 

}

\caption{Control colors for Chord diagram.}\label{fig:chord-diagram-color}
\end{figure}
...
(Figure \ref{fig:chord-diagram-border} A).
...
It can also be a sub matrix (Figure \ref{fig:chord-diagram-border} C).
For rows or columns of which the corresponding values are not specified
in the matrix, default values are filled in. It must have row names and
column names so that the settings can be mapped to the correct links.
...
graphic parameters (Figure \ref{fig:chord-diagram-border} D).
...
{\centering \includegraphics{13-chord-diagram-simple_files/figure-latex/chord-diagram-border-1} 

}

\caption{Control borders for Chord Diagram.}\label{fig:chord-diagram-border}
\end{figure}
...
\ref{fig:chord-diagram-highlight} A).
...
We can also highlight links with values larger than a cutoff (Figure
\ref{fig:chord-diagram-highlight} B). There are at least three ways to
do it. First, construct a color matrix and set links with small values
...
transparency. In following example, links with value less then 12 is set
to \texttt{\#00000000}.
...
\texttt{col\_df} are not drawn (Figure \ref{fig:chord-diagram-highlight}
C).
...
\ref{fig:chord-diagram-highlight} D).
...
{\centering \includegraphics{13-chord-diagram-simple_files/figure-latex/chord-diagram-highlight-1} 

}

\caption{Highlight links by colors.}\label{fig:chord-diagram-highlight}
\end{figure}

\section{Orders of links}\label{orders-of-links}

\subsection{Orders on positions on
sectors}\label{orders-on-positions-on-sectors}
...
\begin{Highlighting}[]
\KeywordTok{chordDiagram}\NormalTok{(mat, }\DataTypeTok{grid.col =}\NormalTok{ grid.col, }\DataTypeTok{transparency =} \DecValTok{0}\NormalTok{)}
\KeywordTok{text}\NormalTok{(}\OperatorTok{-}\FloatTok{0.9}\NormalTok{, }\FloatTok{0.9}\NormalTok{, }\StringTok{"A"}\NormalTok{, }\DataTypeTok{cex =} \FloatTok{1.5}\NormalTok{)}
...
\ref{fig:chord-diagram-directional} A, B, C) to represent the
directions. The position of starting end of the link is shorter than the
other end to give users the feeling that the link is moving out. If this
is not what your correct feeling, you can set \texttt{diffHeight} to a
negative value.
...
\KeywordTok{chordDiagram}\NormalTok{(mat, }\DataTypeTok{grid.col =}\NormalTok{ grid.col, }\DataTypeTok{directional =} \DecValTok{1}\NormalTok{, }\DataTypeTok{diffHeight =} \FloatTok{0.08}\NormalTok{)}
...
(Figure @ref(fig :chord-diagram-directional) D).
...
If you don't need self-link for which two ends of a link are in a same
sector, just set corresponding values to 0 in the matrix (Figure
\ref{fig:chord-diagram-directional} E).
...
\ref{fig:chord-diagram-directional} F). When \texttt{direction.type} is
set to \texttt{arrows}, Arrows are added at the center of links. Similar
as other graphics parameters for links, the parameters for drawing
arrows such as arrow color and line type can either be a scalar, a
matrix, or a three-column data frame.
...
you better visualization (Figure \ref{fig:chord-diagram-directional} G).
...
\ref{fig:chord-diagram-directional} H)
...
longer than the other ends (Figure \ref{fig:chord-diagram-directional}
I)

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{chordDiagram}\NormalTok{(matx, }\DataTypeTok{directional =} \DecValTok{1}\NormalTok{, }\DataTypeTok{direction.type =} \KeywordTok{c}\NormalTok{(}\StringTok{"diffHeight"}\NormalTok{, }\StringTok{"arrows"}\NormalTok{),}
    \DataTypeTok{link.arr.type =} \StringTok{"big.arrow"}\NormalTok{, }\DataTypeTok{diffHeight =} \OperatorTok{-}\FloatTok{0.04}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# code is not run when building the vignette}
...
{\centering \includegraphics{13-chord-diagram-simple_files/figure-latex/chord-diagram-directional-1} 

}

\caption{Visualization of directional matrix.}\label{fig:chord-diagram-directional}
\end{figure}
...
label track and axes. Later, the first track is updated adding labels
with clock wise facings (Figure \ref{fig:chord-diagram-labels} A).
...
\KeywordTok{circos.trackPlotRegion}\NormalTok{(}\DataTypeTok{track.index =} \DecValTok{1}\NormalTok{, }\DataTypeTok{panel.fun =} \ControlFlowTok{function}\NormalTok{(x, y) \{}
...
\ref{fig:chord-diagram-labels} B). Please not \texttt{circos.text()} and
\texttt{get.cell.meta.data()} can be used outside \texttt{panel.fun} if
the sector index and track index are specified explicitly.
...
\ref{fig:chord-diagram-labels} C).
...
{\centering \includegraphics{14-chord-diagram-advanced-usage_files/figure-latex/chord-diagram-labels-1} 

}

\caption{Customize sector labels in Chord diagram.}\label{fig:chord-diagram-labels}
\end{figure}
...
Small gaps between sectors are set to 1 degree and big gaps between row
and column sectors are set to 20 degree.
...
\KeywordTok{highlight.sector}\NormalTok{(}\KeywordTok{rownames}\NormalTok{(mat1), }\DataTypeTok{track.index =} \DecValTok{1}\NormalTok{, }\DataTypeTok{col =} \StringTok{"red"}\NormalTok{, }
    \DataTypeTok{text =} \StringTok{"A"}\NormalTok{, }\DataTypeTok{cex =} \FloatTok{0.8}\NormalTok{, }\DataTypeTok{niceFacing =} \OtherTok{TRUE}\NormalTok{)}
\KeywordTok{highlight.sector}\NormalTok{(}\KeywordTok{colnames}\NormalTok{(mat1), }\DataTypeTok{track.index =} \DecValTok{1}\NormalTok{, }\DataTypeTok{col =} \StringTok{"green"}\NormalTok{, }
    \DataTypeTok{text =} \StringTok{"B"}\NormalTok{, }\DataTypeTok{cex =} \FloatTok{0.8}\NormalTok{,  }\DataTypeTok{niceFacing =} \OtherTok{TRUE}\NormalTok{)}
\KeywordTok{highlight.sector}\NormalTok{(}\KeywordTok{colnames}\NormalTok{(mat2), }\DataTypeTok{track.index =} \DecValTok{1}\NormalTok{, }\DataTypeTok{col =} \StringTok{"blue"}\NormalTok{, }
    \DataTypeTok{text =} \StringTok{"C"}\NormalTok{, }\DataTypeTok{cex =} \FloatTok{0.8}\NormalTok{, }\DataTypeTok{niceFacing =} \OtherTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\item
  ...
  group 1. \texttt{meth\_mat\_1{[}"TssA",\ "TxFlnk"{]}} is the mean
...
\item
  ...
  group 2. \texttt{meth\_mat\_2{[}"TssA",\ "TxFlnk"{]}} is the mean
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{state_col =}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"TssA"}\NormalTok{ =}\StringTok{ "#E41A1C"}\NormalTok{,}
              \StringTok{"TssAFlnk"}\NormalTok{ =}\StringTok{ "#E41A1C"}\NormalTok{,}
              \StringTok{"TxFlnk"}\NormalTok{ =}\StringTok{ "#E41A1C"}\NormalTok{,}
              \StringTok{"Tx"}\NormalTok{ =}\StringTok{ "#E41A1C"}\NormalTok{,}
              \StringTok{"TxWk"}\NormalTok{ =}\StringTok{ "#E41A1C"}\NormalTok{,}
              \StringTok{"EnhG"}\NormalTok{ =}\StringTok{ "#E41A1C"}\NormalTok{,}
              \StringTok{"Enh"}\NormalTok{ =}\StringTok{ "#E41A1C"}\NormalTok{,}
              \StringTok{"ZNF/Rpts"}\NormalTok{ =}\StringTok{ "#E41A1C"}\NormalTok{,}
              \StringTok{"Het"}\NormalTok{ =}\StringTok{ "#377EB8"}\NormalTok{,}
              \StringTok{"TssBiv"}\NormalTok{ =}\StringTok{ "#377EB8"}\NormalTok{,}
              \StringTok{"BivFlnk"}\NormalTok{ =}\StringTok{ "#377EB8"}\NormalTok{,}
              \StringTok{"EnhBiv"}\NormalTok{ =}\StringTok{ "#377EB8"}\NormalTok{,}
              \StringTok{"ReprPC"}\NormalTok{ =}\StringTok{ "#377EB8"}\NormalTok{,}
              \StringTok{"ReprPCWk"}\NormalTok{ =}\StringTok{ "#377EB8"}\NormalTok{,}
...
\NormalTok{cdm_res =}\StringTok{ }\KeywordTok{chordDiagram}\NormalTok{(mat, }\DataTypeTok{col =}\NormalTok{ colmat, }\DataTypeTok{grid.col =}\NormalTok{ state_col2,}
    \DataTypeTok{directional =} \OtherTok{TRUE}\NormalTok{, }\DataTypeTok{annotationTrack =} \StringTok{"grid"}\NormalTok{, }\DataTypeTok{preAllocateTracks =} \KeywordTok{list}\NormalTok{(}\DataTypeTok{track.height =} \FloatTok{0.1}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\begin{center}\includegraphics{15-a-complex-example-with-chord-diagram_files/figure-latex/unnamed-chunk-9-1} \end{center}
...
solved by adding small circular rectangles. In following example, the
newly added circular rectangles in the bottom half shows e.g.~how much
the state 15 in group 1 has been transited to different states in group
2.
...
\KeywordTok{circos.trackPlotRegion}\NormalTok{(}\DataTypeTok{ylim =} \KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{), }\DataTypeTok{bg.border =} \OtherTok{NA}\NormalTok{)}
...
\NormalTok{\}}
\KeywordTok{circos.trackPlotRegion}\NormalTok{(}\DataTypeTok{ylim =} \KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{), }\DataTypeTok{factors =}\NormalTok{ factors, }\DataTypeTok{bg.border =} \OtherTok{NA}\NormalTok{,}
    \DataTypeTok{panel.fun =} \ControlFlowTok{function}\NormalTok{(x, y) \{}
...
##  GlobalOptions   0.0.12  2017-03-25 CRAN (R 3.3.2)                   
##  Rcpp            0.12.9  2017-01-14 CRAN (R 3.3.2)                   
##  backports       1.0.5   2017-01-18 cran (@1.0.5)                    
##  bookdown        0.3.16  2017-03-27 Github (rstudio/bookdown@0b4527c)
##  circlize      * 0.4.0   2017-03-10 CRAN (R 3.3.2)                   
##  colorspace      1.3-2   2016-12-14 cran (@1.3-2)                    
##  devtools        1.12.0  2016-06-24 CRAN (R 3.3.0)                   
##  digest          0.6.12  2017-01-27 cran (@0.6.12)                   
##  evaluate        0.10    2016-10-11 cran (@0.10)                     
##  htmltools       0.3.5   2016-03-21 cran (@0.3.5)                    
##  knitr           1.15.16 2017-03-27 Github (yihui/knitr@9f6a1c2)     
