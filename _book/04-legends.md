...
functionality from other R packages, it is just a few more work to really
...
can see that one big problem is if there are many legends that the size for
...
making grid graphics as well as arranging plotting regions (or, viewport in
