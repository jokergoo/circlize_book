...
of such plot is called Chord Diagram (\url{http://en.wikipedia.org/wiki/Chord_diagram}). 
...
From version 0.3.10, {\tt chordDiagram()} accepts a {\tt link.visible} argument which controls whether to plot
corresponding links. It is faster than using full transparent color because actually no plotting is applied when
values in {\tt link.visible} is {\tt FALES}. The value for {\tt link.visible} is a logical matrix if the input
variable is a matrix and a logial vector is the input variable is a data frame. When corresponding values in 
{\tt link.visible} is {\tt FALSE}, no link is plotted but the position of the link is still occupied.
